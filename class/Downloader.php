...
class Downloader
{
	...
	public function __construct($post)
	{
		...
		foreach ($this->urls as $url)
		{
			...
			{
				$this->errors[] = "\"".$url."\"은 url 주소가 아니에요!";
			}
		}
	...
	private function do_download($audio_only)
	{
		...
		if($audio_only)
		{
			$cmd .= " -f 'ba' -x --audio-format mp3 -o ".$this->download_path."/";
			$cmd .= escapeshellarg($this->outfilename);
		}
		else	
		{	
		$cmd .= " --ignore-error --no-mtime -f bestvideo[ext=mp4]+bestaudio[ext=m4a] -o ".$this->download_path."/";
		$cmd .= escapeshellarg($this->outfilename);
		}
		
		$cmd .= " --no-restrict-filenames"; // --restrict-filenames이 필요할수도~
		foreach($this->urls as $url)
		{
